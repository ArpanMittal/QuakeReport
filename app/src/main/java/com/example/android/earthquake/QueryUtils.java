...
/**
 * Created by arpan on 12/26/2016.
 */
...
public final class QueryUtils {
    ...
    /**
     * url providing list of earthquakes
     */
    ...
    public static List<Earthquake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                float magnitude = Float.parseFloat(jsonObject1.get("mag").toString());
                String place = jsonObject1.getString("place");
                String[] parts = place.split(LOCATION_SEPARATOR);
                if (parts.length < 2) {
                    parts[1] = jsonObject1.getString("Near The");
                }
                String time = jsonObject1.getString("time");
                String url = jsonObject1.getString("url");

                earthquakes.add(new Earthquake(magnitude, parts[0], parts[1], time, url));
            }
